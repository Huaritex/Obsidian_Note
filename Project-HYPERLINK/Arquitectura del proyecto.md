...
> Utilizaremos una **Arquitectura de Micro-Frontends** y una base de datos distribuida mediante **CRDTs** (Conflict-free Replicated Data Types) para garantizar que la edición sea fluida incluso sin conexión.
...
> Para replicar la potencia de Notion Calendar y Obsidian, necesitamos una estructura que soporte tanto archivos planos como bases de datos relacionales.
...
> Para lograr la potencia de Google Notebook, implementaremos una arquitectura de **RAG (Retrieval-Augmented Generation)**.
...
| **Base de Datos**   | SQLite + Yjs              | Estándar de la industria para apps local-first. |
...
- **Grafos Inteligentes:** A diferencia de Obsidian, donde tú creas los enlaces, esta arquitectura usaría la base de datos vectorial para sugerir conexiones entre notas que tú no habías visto (aprendizaje proactivo).
    

> **Nota de Arquitecto:** El mayor reto aquí no es la IA, sino la **latencia**. Leer archivos Markdown, convertirlos a vectores y renderizar un grafo mientras sincronizas con Google Calendar requiere un uso intensivo de _Multi-threading_ en Rust para no bloquear la interfaz de usuario.